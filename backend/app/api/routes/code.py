"""
模块名称：代码练习路由（code）
功能描述：提供代码题库浏览、代码运行和提交判题的全部端点。

端点：
- GET  /code/problems：题目列表（支持难度/标签/关键词筛选）
- GET  /code/problems/{id}：题目详情
- POST /code/problems/{id}/run：运行样例测试
- POST /code/problems/{id}/submit：正式提交判题
- GET  /code/submissions：提交记录列表
- GET  /code/submissions/{id}：提交记录详情

判题流程：
- 运行（run）：仅执行样例用例，同步等待结果
- 提交（submit）：执行全部用例，异步后台判题，提交后立即返回 Running 状态
"""
...
from app.services.judge0_service import LANGUAGE_IDS, Judge0Unavailable, judge0_service, trim_output, truncate_text
...
async def _run_cases(cases: List[models.CodeTestCase], language: str, source_code: str, include_hidden_details: bool, precheck_first: bool = True):
    results = []
...
async def _judge_submission(submission_id: int, case_ids: List[int], language: str, source_code: str):
    ...
    try:
        ...
        try:
            status, passed_count, total_count, results = await _run_cases(cases, language, source_code, False, precheck_first=False)
            error_message = None
        except Judge0Unavailable as exc:
            status, passed_count, total_count = "Judge Error", 0, len(cases)
            error_message = f"判题服务不可用：{exc}"
            results = [schemas.CodeCaseResult(index=1, is_sample=False, passed=False, status=status, message=error_message)]
...
async def run_problem(problem_id: str, payload: schemas.CodeRunRequest, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    """运行代码（仅样例用例），同步等待判题结果。"""
    del user_id
    problem = _get_problem(db, problem_id)
...
async def submit_problem(
    ...
    """正式提交代码（全部用例），异步后台判题，立即返回 Running 状态。"""
    problem = _get_problem(db, problem_id)
    language, source_code = _validate_request(payload)
    cases = (
        db.query(models.CodeTestCase)
...
def list_submissions(problem_id: Optional[int] = Query(None), db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    """获取当前用户的提交记录列表，可按题目筛选。"""
    query = (
...
def get_submission(submission_id: int, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    """获取指定提交记录的详情（含源码和所有测试结果）。"""
    row = db.query(models.CodeSubmission).filter(models.CodeSubmission.id == submission_id, models.CodeSubmission.user_id == user_id).first()
    if not row:
